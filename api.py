...
async def ai_ask(req: AIRequest, x_telegram_init_data: Optional[str] = Header(None)):
    ...
    def _create_message(**kw):
        """Вызов с кэшированием системного промпта и структурированным ответом.
        Если API не примет новые параметры — молча откатываемся на обычный вызов,
        чтобы тренер не замолчал у пользователей из-за неподдержанного поля."""
        try:
            return client.messages.create(**kw)
        except Exception as e:
            msg = str(e)
            if "output_config" in msg or "cache_control" in msg or "schema" in msg.lower():
                logger.warning(f"[ai] откат на простой вызов: {e}")
