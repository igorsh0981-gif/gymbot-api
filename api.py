...
def save_checkin(tg_id: int, req: CheckinRequest):
    with SessionLocal() as db:
        ...
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        db.execute(text("""
            INSERT INTO checkins (user_id, weight, energy_level, sleep_quality, stress_level, motivation_level, created_at)
            VALUES (:uid, :weight, :energy, :sleep, :stress, :motivation, :now)
        """), {"uid": user.id, "weight": req.weight, "energy": req.energy,
               "sleep": req.sleep, "stress": req.stress, "motivation": req.motivation,
               "now": datetime.utcnow().replace(tzinfo=None)})
