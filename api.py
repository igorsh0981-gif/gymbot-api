...
from fastapi import FastAPI, HTTPException, Request as FastAPIRequest, Header, Depends, BackgroundTasks
...
# ── Мониторинг ошибок ───────────────────────────────────────────────
# Sentry получает трассировки, а те по умолчанию содержат локальные переменные.
# В контексте AI-тренера это противопоказания и аллергии пользователя, в путях
# запросов — Telegram ID. Настраиваем так, чтобы персональные данные не уходили:
# локальные переменные выключены, тело запроса не передаётся, id в адресах
# заменяются на заглушку.
_SENTRY_SENSITIVE = ("medical", "allerg", "medication", "password", "token",
                     "api_key", "secret", "init_data", "authorization")


def _sentry_scrub(event, hint):
    """Вычищает персональные данные до отправки события."""
    try:
        # Telegram ID в путях: /api/user/123456789 -> /api/user/<id>
        req = event.get("request") or {}
        if req.get("url"):
            req["url"] = _re_sentry.sub(r"/\d{6,}", "/<id>", req["url"])
        req.pop("data", None)
        req.pop("cookies", None)
        headers = req.get("headers") or {}
        for h in list(headers):
            if h.lower() in ("authorization", "x-telegram-init-data", "cookie"):
                headers[h] = "<removed>"
        event["request"] = req

        # На всякий случай — если где-то всё же просочились переменные
        for entry in (event.get("exception") or {}).get("values") or []:
            for frame in (entry.get("stacktrace") or {}).get("frames") or []:
                ...
                if not v:
                    continue
                for name in list(v):
                    if any(s in name.lower() for s in _SENTRY_SENSITIVE):
                        v[name] = "<removed>"
        event.pop("user", None)
        return event
    except Exception:
        # Если чистка почему-то сломалась — лучше не отправлять ничего
        return None
...
try:
    ...
    if _SENTRY_DSN:
        _sentry.init(
            ...
            include_local_variables=False,   # ключевое: не отправлять значения переменных
        ...
        logger.info("[sentry] мониторинг ошибок включён, персональные данные вычищаются")
except ImportError:
    pass
except Exception as _e:
    logger.warning(f"[sentry] не удалось включить: {_e}")

# Цены Claude Sonnet (актуальны на момент внедрения — при смене модели/тарифа поправить)
_AI_PRICE_INPUT_PER_M = 3.0
_AI_PRICE_OUTPUT_PER_M = 15.0

def _log_ai_usage(db, user_id, endpoint: str, response):
    """Логирует расход токенов и стоимость одного запроса к Anthropic API.
    Вызывать сразу после каждого client.messages.create(). Не должно ронять
    основной запрос, если логирование почему-то не удалось."""
    try:
        ...
        cost = (in_tok / 1_000_000 * _AI_PRICE_INPUT_PER_M) + (out_tok / 1_000_000 * _AI_PRICE_OUTPUT_PER_M)
        db.execute(text("""
            INSERT INTO ai_usage_log (user_id, endpoint, input_tokens, output_tokens, cost_usd)
            VALUES (:uid, :ep, :itok, :otok, :cost)
        """), {"uid": user_id, "ep": endpoint, "itok": in_tok, "otok": out_tok, "cost": cost})
        db.commit()
    except Exception as e:
        logger.warning(f"[ai_usage] Не удалось залогировать расход: {e}")
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "")
AI_DAILY_LIMIT = 5

# ─── Telegram WebApp initData verification ───────────────────────────────────
# Без этого любой мог передать чужой tg_id в URL и читать/менять чужие данные —
# Telegram подписывает initData секретным ключом на основе токена бота,
# мы проверяем подпись и берём user id ТОЛЬКО из проверенных данных, а не из
# параметра запроса.
BOT_TOKEN_FOR_AUTH = os.getenv("BOT_TOKEN", "")
_INIT_DATA_MAX_AGE_SEC = 86400  # initData считается свежим не дольше суток

def _verify_telegram_init_data(init_data: str) -> Optional[dict]:
    """Проверяет подпись initData от Telegram WebApp. Возвращает распарсенные
    данные (включая user) если подпись верна и данные не устарели, иначе None."""
    if not init_data or not BOT_TOKEN_FOR_AUTH:
        return None
    try:
        ...
        secret_key = hmac.new(b"WebAppData", BOT_TOKEN_FOR_AUTH.encode(), hashlib.sha256).digest()
        ...
        if auth_date and (datetime.utcnow().timestamp() - auth_date) > _INIT_DATA_MAX_AGE_SEC:
            return None
        ...
        logger.warning(f"[auth] initData verification error: {e}")
        return None


def verify_tg_user(tg_id: int, x_telegram_init_data: Optional[str] = Header(None)) -> int:
    """FastAPI-зависимость: проверяет, что tg_id в пути запроса совпадает
    с реальным, криптографически подтверждённым пользователем Telegram.
    Без валидного initData, при несовпадении id, или при отсутствии BOT_TOKEN — 403.
    Намеренно fail-closed: если проверку по какой-то причине нельзя выполнить,
    правильное поведение — отказать в доступе, а не молча его разрешить."""
    if not BOT_TOKEN_FOR_AUTH:
        logger.error("[auth] BOT_TOKEN not set — blocking all requests until it's configured!")
        raise HTTPException(status_code=503, detail="Server auth misconfigured: BOT_TOKEN missing")

    verified = _verify_telegram_init_data(x_telegram_init_data or "")
    if not verified or not verified.get("user"):
        raise HTTPException(status_code=403, detail="Invalid or missing Telegram authentication")

    verified_tg_id = verified["user"].get("id")
    if verified_tg_id != tg_id:
        raise HTTPException(status_code=403, detail="tg_id does not match authenticated Telegram user")

    return tg_id

def verify_tg_body(claimed_tg_id: Optional[int], x_telegram_init_data: Optional[str]) -> Optional[int]:
    """То же, что verify_tg_user, но для tg_id из тела POST-запроса.

    Зависимость verify_tg_user читает tg_id только из пути или query, поэтому
    POST-эндпоинты с tg_id внутри JSON оставались без проверки: можно было
    подставить чужой id и получить ответ, собранный из чужих данных.
    """
    if claimed_tg_id is None:
        return None
    return verify_tg_user(claimed_tg_id, x_telegram_init_data)
...
def r2_video_url(slug: str, video_ext: Optional[str]) -> Optional[str]:
    if not slug or not video_ext or not R2_PUBLIC_URL:
        return None
    from urllib.parse import quote
    version = int(datetime.utcnow().timestamp())
    return f"{R2_PUBLIC_URL}/{quote(f'exercises/{slug}/demo.{video_ext}', safe='/')}?v={version}"


def r2_group_icon_url(group_id: int, icon_ext: Optional[str]) -> Optional[str]:
    if not icon_ext or not R2_PUBLIC_URL:
        return None
    from urllib.parse import quote
    version = int(datetime.utcnow().timestamp())
    return f"{R2_PUBLIC_URL}/{quote(f'muscle_groups/{group_id}/icon.{icon_ext}', safe='/')}?v={version}"
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://igorsh0981-gif.github.io"],
...
@app.get("/api/user/{tg_id}", dependencies=[Depends(verify_tg_user)])
    ...
    with SessionLocal() as db:
        user = db.execute(text("""
            SELECT id, telegram_id, first_name, username,
                   ...
                   consent_given_at, consent_version, consent_platform, consent_username,
        ...
        # Дней с последней завершённой тренировки — нужно для выбора варианта приветствия
        # на главном экране (давно не заходил / тренировался сегодня / держит серию).
        _last_wk = db.execute(text("""
            SELECT MAX(DATE(date)) FROM workouts WHERE user_id=:uid AND status='finished'
        """), {"uid": user.id}).scalar()
        days_since_workout = (datetime.utcnow().date() - _last_wk).days if _last_wk else None
        # Счётчик запросов к AI сбрасывается лениво — только когда пользователь реально
        # отправляет запрос в /api/ai/ask. Если он просто открыл экран, не отправив ничего
        # сегодня, сырое значение в БД может быть вчерашним. Проверяем дату здесь же,
        # чтобы отображаемое значение было всегда актуальным, не дожидаясь запроса.
        _reset_date = user.ai_requests_reset_date
        if hasattr(_reset_date, "date"):
            _reset_date = _reset_date.date()
        _effective_ai_requests_today = (user.ai_requests_today or 0) if _reset_date == datetime.utcnow().date() else 0
        return {
            ...
            "consent_username": user.consent_username,
            "consents": {
                "terms": user.consent_terms_at.isoformat() if user.consent_terms_at else None,
                "basic": user.consent_basic_at.isoformat() if user.consent_basic_at else None,
                "health": user.consent_health_at.isoformat() if user.consent_health_at else None,
                "ai": user.consent_ai_at.isoformat() if user.consent_ai_at else None,
            },
            "ai_requests_today": _effective_ai_requests_today,
...
@app.get("/api/workouts/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/stats/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
def get_exercises(group_id: Optional[int] = None, search: Optional[str] = None):
    with SessionLocal() as db:
        ...
        """), params).fetchall()
        groups = db.execute(text("SELECT id, name, emoji, icon_ext, COALESCE(name_en, name) as name_en, COALESCE(name_uz, name) as name_uz, COALESCE(name_kz, name) as name_kz FROM muscle_groups ORDER BY id")).fetchall()
        return {
            "exercises": [{"id": e.id, "name": e.name, "description": e.description,
                ...
                "group_emoji": e.group_emoji, "group_icon_url": r2_group_icon_url(e.muscle_group_id, getattr(e,"group_icon_ext",None)), "photo_url": r2_photo_url(e.r2_slug),
                "video_url": r2_video_url(e.r2_slug, getattr(e, "video_ext", None)),
                "exercise_type": getattr(e, 'exercise_type', 'strength') or 'strength'} for e in exercises],
            "muscle_groups": [{"id": g.id, "name": g.name, "emoji": g.emoji,
                "icon_url": r2_group_icon_url(g.id, getattr(g, "icon_ext", None)),
                "name_en": getattr(g, "name_en", None), "name_uz": getattr(g, "name_uz", None), "name_kz": getattr(g, "name_kz", None)} for g in groups],
        }
...
async def ai_ask(req: AIRequest, x_telegram_init_data: Optional[str] = Header(None)):
    ...
    # Без этой проверки можно было подставить чужой tg_id и получить ответ,
    # собранный из чужой истории тренировок, весов и медпоказаний.
    verify_tg_body(req.tg_id, x_telegram_init_data)
    # Документ обещает: без согласия на передачу в Anthropic AI-функции недоступны
    if req.tg_id:
        with SessionLocal() as _cdb:
            _u = _cdb.execute(text("SELECT id FROM users WHERE telegram_id=:t"), {"t": req.tg_id}).fetchone()
            if _u:
                _require_consent(_cdb, _u.id, "ai")
    # skip_limit обходит суточный лимит запросов — из внешнего запроса недопустимо
    req.skip_limit = False
            ...
            try:
                ...
                if recent_wk:
                    ...
                    for wk in recent_wk:
                        ...
                        wk_parts.append((wk_str, wk.ai_review))
                    if wk_parts:
                        # Для САМОЙ ПОСЛЕДНЕЙ тренировки отзыв тренера НЕ обрезаем — именно там
                        # обычно содержится критичная инструкция вроде "следующая тренировка — деload".
                        # Обрезка в 80 символов теряла это почти всегда, так как такая инструкция
                        # обычно идёт в конце длинного отзыва.
                        last_str, last_review = wk_parts[0]
                        if last_review:
                            last_str += f"\nTRAINER'S FULL NOTE AFTER THIS WORKOUT (follow this — if it says deload/lighter session, the next workout you propose MUST be that, not a normal/heavy one): {last_review}"
                        workouts_context = "\n\nTODAY'S WORKOUT (most recent, analyze this specifically):\n" + last_str
                        if len(wk_parts) > 1:
                            older_parts = []
                            for s, rev in wk_parts[1:]:
                                if rev: s += f" | prev advice: {rev[:200]}..."
                                older_parts.append(s)
                            workouts_context += "\n\nPREVIOUS WORKOUTS:\n" + "\n".join(older_parts)
                ...
                prog = db.execute(text("""
                    ...
                    GROUP BY ws.exercise_name ORDER BY last_d DESC, sessions DESC LIMIT 40
                """), {"uid": user.id}).fetchall()
                if prog:
    ...
    medical_str = ""
    allergies_str = ""
    # Медицинские сведения уходят в Anthropic только при согласии именно на их
    # обработку. Отозвал согласие на здоровье — противопоказания в запрос не попадают,
    # даже если AI-функции в целом разрешены.
    _health_ok = False
    if user:
        try:
            with SessionLocal() as _hdb:
                _hr = _read_consents(_hdb, user.id)
                _health_ok = bool(_hr and _hr.consent_health_at)
        except Exception as _e:
            logger.warning(f"[consent] не удалось проверить согласие на здоровье: {_e}")
    if user and _health_ok:
        try:
    ...
    if user:
        context = (
            ...
            f"and do not comment that weights are lower than before, since lower weights were explicitly recommended. "
        ...
        if is_workout_question:
            context += (
                "\n\nIMPORTANT FORMATTING: For each exercise you comment on, write a SEPARATE short paragraph "
                "that STARTS with the exercise name (exact name from catalog), followed by your comment for "
                "THAT exercise only. Put a blank line between each exercise's paragraph. Do NOT combine multiple "
                "exercises into one flowing sentence or paragraph — each exercise needs its own distinct paragraph "
                "so the app can show the right comment under the right exercise. Keep each paragraph brief (1-2 sentences) "
                "— you MUST leave room to also output the JSON block below.\n"
                "After your response add this on a NEW LINE with ONLY these exercises (max 8, exact names from catalog):\n"
    ...
    # Схема ответа для вопросов про тренировку. Совет по каждому упражнению —
    # отдельное поле, поэтому он не может ни обрезаться, ни прилипнуть к соседнему,
    # как это было при вытаскивании регуляркой из прозы.
    _PLAN_SCHEMA = {
        "type": "object",
        "properties": {
            "answer": {"type": "string", "description": "Ответ тренера человеку, обычным текстом"},
            "workout_plan": {
                "type": "object",
                "properties": {
                    "title": {"type": "string", "description": "Короткое название, пусто если плана нет"},
                    "exercises": {
                        "type": "array",
                        "items": {
                            "type": "object",
                            "properties": {
                                "name": {"type": "string", "description": "Точное название из каталога"},
                                "tip": {"type": "string", "description": "Совет по этому упражнению: вес, повторы, техника"},
                            },
                            "required": ["name", "tip"],
        ...
        "required": ["answer", "workout_plan"],
        "additionalProperties": False,
    }

    def _create_message(_depth=0, **kw):
        """Вызов с кэшированием системного промпта и структурированным ответом.

        Откат пошаговый: сначала отключаем только то, что не приняли. Раньше
        отваливалось всё сразу, и из-за неподдержанного output_config терялось
        ещё и кэширование промпта, хотя оно работало.
        """
        try:
            ...
            if ("output_config" in msg or "schema" in msg.lower()) and "output_config" in kw:
                logger.warning(f"[ai] схема ответа не поддержана, отключаю только её: {e}")
            ...
            if ("cache_control" in msg or "system" in msg.lower()) and isinstance(kw.get("system"), list):
                logger.warning(f"[ai] кэширование промпта не поддержано, отключаю: {e}")
    ...
    _kw = {
        ...
        "max_tokens": 4000 if (system_prompt and chat_history) else 1600,
        "messages": messages,
    }
    if system_prompt and chat_history:
        # Системный промпт почти не меняется между запросами — кэшируем его
        _kw["system"] = [{"type": "text", "text": system_prompt,
                          "cache_control": {"type": "ephemeral"}}]
    if is_workout_question:
        _kw["output_config"] = {"format": {"type": "json_schema", "schema": _PLAN_SCHEMA}}
    ...
    try:
        ...
        _cache = getattr(_u, "cache_read_input_tokens", 0) or 0
        logger.info(
            f"[ai timing] контекст из базы {(_t_ctx - _t0):.1f}с | "
            f"ответ Anthropic {(_t_api - _t_ctx):.1f}с | "
            f"токены вход {_in} (из кэша {_cache}), выход {_out} | "
            f"схема {'да' if 'output_config' in _kw else 'нет'}"
        )
    except Exception:
        pass
    try:
        with SessionLocal() as _usage_db:
            _log_ai_usage(_usage_db, user.id if user else None, "ai_ask", message)
    except Exception as _e:
        logger.warning(f"[ai_usage] Пропущено логирование расхода (не критично): {_e}")
    ...
    # Структурированный ответ: план и советы приходят полями, разбирать нечего
    if is_workout_question:
        try:
            _s = _json.loads(raw_answer)
            _plan = _s.get("workout_plan") or {}
            _exs = [e for e in (_plan.get("exercises") or []) if e.get("name")]
            answer = (_s.get("answer") or "").strip()
            if _exs:
                workout_plan = {
                    "title": (_plan.get("title") or "AI тренировка").strip(),
                    "exercises": [e["name"] for e in _exs],
                    "tips": {e["name"]: (e.get("tip") or "").strip() for e in _exs},
                }
            _parsed_structured = True
        except Exception as _e:
            # Ответ пришёл прозой — значит сработал откат, идём старым путём
            logger.warning(f"[ai] структурированный ответ не разобран, разбираю как текст: {_e}")
            _parsed_structured = False
    else:
        _parsed_structured = False

    plan_match = None if _parsed_structured else _re.search(r"WORKOUT_PLAN_JSON:\s*(\{.+?\})", raw_answer, _re.DOTALL)
    if _parsed_structured:
        pass
    elif plan_match:
        try:
...
@app.delete("/api/ai/history/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/nutrition/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.put("/api/user/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/planned/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
class PlannedWorkoutRequest(BaseModel):
    ...
    exercise_ids: Optional[List[int]] = None  # строго числа: значения уходят в SQL
    ...
    exercise_tips: Optional[dict] = None  # готовые советы {название упражнения: совет} из структурированного ответа

@app.post("/api/planned/{tg_id}", dependencies=[Depends(verify_tg_user)])
def create_planned(tg_id: int, req: PlannedWorkoutRequest):
    with SessionLocal() as db:
        ...
        # Советы пришли готовыми полями от модели — разбирать прозу не нужно.
        # Разбор абзацев остаётся только как запасной путь для старых клиентов.
        if req.exercise_tips and exercise_ids:
            try:
                rows_t = db.execute(text(f"""
                    SELECT id, name FROM exercises WHERE id IN ({",".join(str(int(i)) for i in exercise_ids)})
                """)).fetchall()
                by_name = {r.name.strip().lower(): r.id for r in rows_t}
                for ex_name, tip in req.exercise_tips.items():
                    eid = by_name.get((ex_name or "").strip().lower())
                    if eid and tip:
                        exercise_tips[str(eid)] = str(tip)[:500]
            except Exception as ex_err:
                logger.warning(f"Не удалось разложить готовые советы: {ex_err}")

        if not exercise_tips and req.ai_plan_text and exercise_ids:
            try:
                import re as _re
                ex_rows = db.execute(text(f"""
                    SELECT id, name FROM exercises WHERE id IN ({",".join(str(int(i)) for i in exercise_ids)})
                """)).fetchall()
                # Разбиваем по двойному переносу, нумерованным/маркированным спискам, а также
                # по одиночному переносу — AI нередко разделяет советы одним \n вместо пустой строки,
                # и тогда весь ответ оставался единым куском и прилипал к одному упражнению.
                paragraphs = [p.strip() for p in _re.split(r"\n\n+|\n(?=\d+\.\s)|\n(?=[-—*]\s)|\n", req.ai_plan_text) if p.strip()]
                for ex_row in ex_rows:
                    ...
                    match_para = None
                    matched_by_start = False
                    for para in paragraphs:
                        # Даже если абзац начинается с названия упражнения, но при этом очень длинный —
                        # это общий блок на всю тренировку (AI не разбил ответ), а не совет по одному
                        # упражнению. Иначе весь текст ответа прилипал бы к первому же упражнению.
                        if len(para) > 600:
                            continue
                        ...
                        if para_clean_start.startswith(ex_normalized) or (ex_short and para_clean_start.startswith(ex_short)):
                            match_para = para
                            matched_by_start = True
                            break
                    if not match_para:
                        for para in paragraphs:
                            para_lower = para.lower()
                            if ex_normalized in para_lower or (ex_short and ex_short in para_lower):
                                # Совпадение только "где-то в тексте" (не в начале абзаца) на длинном
                                # абзаце обычно значит, что это общий блок на несколько упражнений сразу
                                # (AI не разбил ответ по абзацам, как просили) — такое не показываем,
                                # чтобы не выводить чужой совет под этим упражнением.
                                if len(para) > 350:
                                    continue
                                match_para = para
                                break
...
@app.delete("/api/planned/{tg_id}/{workout_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/goals/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/goals/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/reminders/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/workout/{workout_id}", dependencies=[Depends(verify_tg_user)])
...
class StartWorkoutRequest(BaseModel):
    ...
    exercise_ids: Optional[List[int]] = None  # список id упражнений, строго числа

@app.post("/api/workout/start/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/workout/{workout_id}/finish", dependencies=[Depends(verify_tg_user)])
def finish_workout(workout_id: int, req: FinishWorkoutRequest, tg_id: int):
    with SessionLocal() as db:
        ...
        new_achs = check_and_grant_achievements(db, user.id, workout_id=workout_id)
        db.commit()
        return {
            ...
            for i, eid in enumerate(ex_ids):
                ...
                if r:
                    exercises.append({
                        ...
                        "group_icon_url": r2_group_icon_url(r.e_mgid, r.mg_icon_ext) if r.e_mgid else None,
...
@app.put("/api/planned/{tg_id}/{workout_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/nutrition/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/measurements/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/measurements/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/checkin/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
async def create_support_ticket(req: SupportRequest, x_telegram_init_data: Optional[str] = Header(None)):
    # Иначе можно было создавать обращения от имени любого пользователя
    verify_tg_body(req.tg_id, x_telegram_init_data)
    import httpx, os
...
Правила: используй указанный вес порции. Если несколько компонентов — суммируй. Отвечай ТОЛЬКО валидным JSON."""
    ...
    try:
        ...
        try:
            with SessionLocal() as _usage_db:
                _log_ai_usage(_usage_db, None, "food_recalc", response)  # tg_id не передаётся в этот эндпоинт
        except Exception as _e:
            logger.warning(f"[ai_usage] Пропущено логирование расхода (не критично): {_e}")
...
def analyze_food_photo(req: FoodPhotoRequest, x_telegram_init_data: Optional[str] = Header(None)):
    # Без проверки можно было записать произвольную еду в чужой дневник питания
    verify_tg_body(req.tg_id, x_telegram_init_data)
    if req.tg_id:
        with SessionLocal() as _cdb:
            _u = _cdb.execute(text("SELECT id FROM users WHERE telegram_id=:t"), {"t": req.tg_id}).fetchone()
    ...
    try:
        ...
        try:
            with SessionLocal() as _usage_db:
                _uid_for_log = None
                if req.tg_id:
                    _u = _usage_db.execute(text("SELECT id FROM users WHERE telegram_id=:tid"), {"tid": req.tg_id}).fetchone()
                    _uid_for_log = _u.id if _u else None
                _log_ai_usage(_usage_db, _uid_for_log, "food_analyze", response)
        except Exception as _e:
            logger.warning(f"[ai_usage] Пропущено логирование расхода (не критично): {_e}")
        raw = response.content[0].text.strip()
        # Убираем markdown
...
@app.delete("/api/nutrition/{tg_id}/{log_id}", dependencies=[Depends(verify_tg_user)])
...
@app.put("/api/nutrition/{tg_id}/{log_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/nutrition/{tg_id}/water", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/nutrition/{tg_id}/history", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/workout/{workout_id}/ai-review", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/workouts/{tg_id}/recent-reviews", dependencies=[Depends(verify_tg_user)])
...
# ════════════════════════════════════════════════════════════════════
#  ГЛАВНЫЙ ЭКРАН — недельный план, карточка тренера, альтернативы
# ════════════════════════════════════════════════════════════════════

# Системы сплитов. Каждый блок — один тренировочный день.
# "auto" — текущее поведение: группы выбираются по времени отдыха, без схемы.
import json as _json_dr

SPLIT_SYSTEMS = {
    "auto":           {"name": "Произвольный",                      "blocks": None},
    "fullbody":       {"name": "Фулбоди",                           "blocks": [
        ["Грудные мышцы", "Спина", "Ноги", "Плечи", "Пресс"],
    ]},
    "upper_lower":    {"name": "Верх / низ",                        "blocks": [
        ["Грудные мышцы", "Спина", "Плечи", "Бицепс", "Трицепс"],
        ["Ноги", "Ягодицы", "Пресс"],
    ]},
    "push_pull_legs": {"name": "Жим / тяга / ноги",                 "blocks": [
        ["Грудные мышцы", "Плечи", "Трицепс"],
        ["Спина", "Бицепс"],
        ["Ноги", "Ягодицы"],
    ]},
    "four_day":       {"name": "Грудь / спина / ноги / руки",       "blocks": [
        ["Грудные мышцы", "Трицепс"],
        ["Спина", "Бицепс"],
        ["Ноги", "Ягодицы"],
        ["Плечи", "Пресс"],
    ]},
    "antagonist":     {"name": "Спина-бицепс / грудь-трицепс / ноги-плечи", "blocks": [
        ["Спина", "Бицепс"],
        ["Грудные мышцы", "Трицепс"],
        ["Ноги", "Плечи"],
    ]},
...
# Сочетания для режима "auto". Список курируемый, а не собранный механически:
# иначе две самые отдохнувшие группы могли дать связку вроде "ноги + спина" —
# два самых объёмных дня в одной тренировке.
AUTO_PAIRS = [
    ["Спина", "Бицепс"],
    ["Грудные мышцы", "Трицепс"],
    ["Ноги", "Ягодицы"],
...
def _group_rest_days(db, user_id: int) -> dict:
    """Сколько дней назад каждая группа мышц тренировалась последний раз.
    Группа, которой нет в истории, получает большое число — значит самая отдохнувшая."""
    rows = db.execute(text("""
        SELECT mg.id AS gid, mg.name AS gname, MAX(DATE(w.date)) AS last_d
        FROM workout_sets ws
        JOIN workouts w ON w.id = ws.workout_id
        JOIN exercises e ON (e.id = ws.exercise_id OR (ws.exercise_id IS NULL AND LOWER(e.name) = LOWER(ws.exercise_name)))
        JOIN muscle_groups mg ON mg.id = e.muscle_group_id
        WHERE w.user_id = :uid AND w.status = 'finished'
    ...
    for r in rows:
        if r.last_d:
            last_by_name[r.gname] = (today - r.last_d).days

    all_groups = db.execute(text("SELECT id, name FROM muscle_groups ORDER BY id")).fetchall()
    out = {}
    for g in all_groups:
        if g.name in _NON_STRENGTH_GROUPS:
            continue
        # 99 = никогда не тренировалась
        out[g.name] = {"id": g.id, "rest_days": last_by_name.get(g.name, 99)}
    return out


def _week_summary(db, user_id: int, target: int = None) -> dict:
    """Что сделано на этой неделе: силовые тренировки и спортивные занятия по дням."""
    monday = _week_monday()
    gym = db.execute(text("""
        SELECT DISTINCT DATE(date) AS d FROM workouts
        WHERE user_id = :uid AND status = 'finished' AND DATE(date) >= :mon
    """), {"uid": user_id, "mon": monday}).fetchall()
    sport = db.execute(text("""
        SELECT session_date AS d, sport_type FROM sport_sessions
        WHERE user_id = :uid AND session_date >= :mon
    """), {"uid": user_id, "mon": monday}).fetchall()

    gym_days = {r.d for r in gym if r.d}
    sport_days = {r.d for r in sport if r.d}
    # Вид спорта по дню: безымянная точка в интерфейсе читается как пропуск
    sport_by_day = {}
    for r in sport:
        if r.d and r.sport_type and r.d not in sport_by_day:
            sport_by_day[r.d] = r.sport_type
    today = datetime.utcnow().date()
    ...
    for i in range(7):
        ...
        days.append({
            ...
            "weekday": i,
            "gym": d in gym_days,
            "sport": d in sport_days,
            "sport_type": sport_by_day.get(d),
            "is_today": d == today,
        })
    return {
        "monday": str(monday),
        "done": len(gym_days),          # спорт в квоту не входит
        "sport_count": len(sport_days),
        "target": target,
        "days": days,
    }


def _groups_trained_this_week(db, user_id: int) -> dict:
    """Какие группы мышц уже отработаны на этой неделе и в какой день."""
    monday = _week_monday()
    rows = db.execute(text("""
        SELECT mg.name AS gname, MAX(DATE(w.date)) AS d
        FROM workout_sets ws
        JOIN workouts w ON w.id = ws.workout_id
        JOIN exercises e ON (e.id = ws.exercise_id OR (ws.exercise_id IS NULL AND LOWER(e.name) = LOWER(ws.exercise_name)))
        JOIN muscle_groups mg ON mg.id = e.muscle_group_id
        WHERE w.user_id = :uid AND w.status = 'finished' AND DATE(w.date) >= :mon
        GROUP BY mg.name
    """), {"uid": user_id, "mon": monday}).fetchall()
    return {r.gname: str(r.d) for r in rows if r.d}


def _build_split_options(db, user_id: int, split_type: str) -> list:
    """Список вариантов тренировки, отсортированный по отдыху групп.
    Каждый вариант помечен, отрабатывался ли он уже на этой неделе."""
    rest = _group_rest_days(db, user_id)
    trained = _groups_trained_this_week(db, user_id)

    system = SPLIT_SYSTEMS.get(split_type or "auto", SPLIT_SYSTEMS["auto"])
    blocks = system["blocks"]

    if not blocks:
        # Произвольный: берём готовые сочетания, ранжирование ниже сделает остальное
        blocks = [list(p) for p in AUTO_PAIRS]

    options = []
    for blk in blocks:
        blk = [g for g in blk if g in rest]
        if not blk:
            continue
        min_rest = min(rest[g]["rest_days"] for g in blk)
        done_days = [trained[g] for g in blk if g in trained]
        options.append({
            "key": "+".join(blk),
            "title": " и ".join(blk[:2]) if len(blk) <= 2 else ", ".join(blk),
            "groups": blk,
            "group_ids": [rest[g]["id"] for g in blk],
            "min_rest_days": min_rest,
            # Признак "было на этой неделе" оказался бесполезным: при 4-5 тренировках
            # в неделю задеты почти все группы, и все варианты попадали в один серый
            # список. Значимо другое — успела ли мышца восстановиться.
            "recently_trained": min_rest <= 1,
            "done_this_week": bool(done_days),
            "done_date": max(done_days) if done_days else None,
        })

    # Сначала восстановившееся, внутри — по убыванию отдыха
    options.sort(key=lambda o: (o["recently_trained"], -o["min_rest_days"]))
    return options


def _rec_description(db, user_id: int, option: dict, week: dict) -> str:
    """Текст рекомендации из фактов, без AI. Никаких выдуманных цифр."""
    parts = []
    rest = option["min_rest_days"]
    main = option["groups"][0]
    if rest >= 99:
        parts.append(f"{main} ещё не тренировались — начнём с умеренных весов.")
    elif rest >= 5:
        parts.append(f"{main.capitalize()} отдыхали {rest} дн. — самая отдохнувшая группа.")
    elif rest >= 2:
        parts.append(f"{main.capitalize()} отдыхали {rest} дн., восстановление нормальное.")
    else:
        parts.append(f"{main.capitalize()} тренировались вчера — сегодня лучше взять другую группу.")

    # Спорт на этой неделе влияет на нагрузку
    monday = _week_monday()
    sport = db.execute(text("""
        SELECT sport_type, session_date FROM sport_sessions
        WHERE user_id = :uid AND session_date >= :mon
        ORDER BY session_date DESC LIMIT 1
    """), {"uid": user_id, "mon": monday}).fetchone()
    if sport and sport.session_date:
        days_ago = (datetime.utcnow().date() - sport.session_date).days
        if days_ago <= 2 and any(g in ("Ноги", "Ягодицы") for g in option["groups"]):
            parts.append("Недавно был спорт — ноги уже получили нагрузку, не лезь в рекорды.")

    if week.get("target") and week.get("done") is not None:
        left = week["target"] - week["done"]
        if left > 0:
            parts.append(f"Это {week['done'] + 1}-я силовая из {week['target']} на неделе.")
    return " ".join(parts)


def _generate_daily_recommendation(user_id: int, tg_id: int, option: dict, week: dict):
    """Пишет текст рекомендации на сегодня и кладёт в daily_recommendations.

    Запускается фоном при первом за день открытии главного экрана — так человек
    сразу видит экран, а текст появляется к следующему обновлению. Контекст здесь
    компактный, а не полная история: задача короткая, и платить за длинный
    промпт каждому активному пользователю ежедневно незачем.
    """
    if not ANTHROPIC_API_KEY:
        return
    try:
        ...
        with SessionLocal() as db:
            exists = db.execute(text("""
                SELECT 1 FROM daily_recommendations WHERE user_id = :uid AND rec_date = :d
            """), {"uid": user_id, "d": today}).fetchone()
            if exists:
                return

            u = db.execute(text("""
                SELECT first_name, age, weight, fitness_level, desired_result FROM users WHERE id = :uid
            """), {"uid": user_id}).fetchone()

            # Последняя тренировка тех же групп — чтобы было от чего отталкиваться в весах
            gids = option.get("group_ids") or []
            last_sets = []
            if gids:
                last_sets = db.execute(text(f"""
                    SELECT ws.exercise_name, MAX(ws.weight) AS w, MAX(ws.reps) AS r
                    FROM workout_sets ws
                    JOIN workouts w ON w.id = ws.workout_id
                    JOIN exercises e ON e.id = ws.exercise_id
                    WHERE w.user_id = :uid AND w.status = 'finished'
                      AND e.muscle_group_id IN ({",".join(str(int(g)) for g in gids)})
                      AND w.date >= NOW() - INTERVAL '45 days'
                    GROUP BY ws.exercise_name ORDER BY MAX(w.date) DESC LIMIT 8
                """), {"uid": user_id}).fetchall()

            sport = db.execute(text("""
                SELECT sport_type, session_date FROM sport_sessions
                WHERE user_id = :uid AND session_date >= NOW() - INTERVAL '7 days'
                ORDER BY session_date DESC LIMIT 3
            """), {"uid": user_id}).fetchall()

        hist = "; ".join(f"{r.exercise_name} {r.w}кг×{r.r}" for r in last_sets if r.w) or "нет данных"
        sport_str = "; ".join(f"{s.sport_type} {s.session_date}" for s in sport) or "не было"
        goal = u.desired_result if u else ""

        prompt = (
            f"Ты тренер. Напиши 2-3 коротких предложения, почему сегодня стоит тренировать "
            f"именно это и с какой интенсивностью. Без приветствий и воды, обращайся на ты.\n"
            f"Тренировка: {option.get('title')}\n"
            f"Дней отдыха у целевых групп: {option.get('min_rest_days')}\n"
            f"Силовых на неделе: {week.get('done')} из {week.get('target') or 'без плана'}\n"
            f"Спорт за неделю: {sport_str}\n"
            f"Последние веса в этих группах: {hist}\n"
            f"Цель: {goal}. Уровень: {u.fitness_level if u else ''}."
        )

        client = _an.Anthropic(api_key=ANTHROPIC_API_KEY)
        msg = client.messages.create(
            model="claude-haiku-4-5-20251001", max_tokens=300,
            messages=[{"role": "user", "content": prompt}],
        )
        try:
            with SessionLocal() as _ud:
                _log_ai_usage(_ud, user_id, "daily_rec", msg)
        except Exception:
            pass

        descr = msg.content[0].text.strip()
        if not descr:
            return
        ...
        with SessionLocal() as db:
            db.execute(text("""
                INSERT INTO daily_recommendations
                    (user_id, rec_date, title, description, split_key, muscle_group_ids, est_minutes, status)
                VALUES (:uid, :d, :t, :descr, :sk, :gids, :em, 'active')
                ON CONFLICT (user_id, rec_date) DO NOTHING
            """), {"uid": user_id, "d": today, "t": option.get("title"), "descr": descr,
                   "sk": option.get("key"), "gids": _json_dr.dumps(gids),
                   "em": 60 + 15 * (len(option.get("groups") or [1]) - 1)})
            db.commit()
        logger.info(f"[daily_rec] Текст на {today} сгенерирован для user_id={user_id}")
    except Exception as e:
        logger.warning(f"[daily_rec] Не удалось сгенерировать: {e}")


@app.get("/api/home/{tg_id}", dependencies=[Depends(verify_tg_user)])
def get_home(tg_id: int, background: BackgroundTasks = None):
    """Всё для главного экрана одним запросом: шапка, неделя, карточка тренера, статистика."""
    with SessionLocal() as db:
        user = db.execute(text("""
            SELECT id, first_name, desired_result, profile_complete, weight,
                   target_workouts_per_week, split_type
        ...
        week = _week_summary(db, user.id, user.target_workouts_per_week)

        # ── Карточка: определяем состояние ──────────────────────────────
        card = None
        ...
        if not user.profile_complete:
            card = {
                ...
                "description": "Без веса, роста и уровня подготовки я не могу подобрать тебе рабочие веса. Это займёт минуту.",
                "cta": "Заполнить профиль",
            }

        if card is None:
            # Только свежая незавершённая тренировка. Записи со статусом active
            # могут висеть в базе неделями — приложение закрыли, не завершив сессию.
            # Предлагать «продолжить» вчерашнюю тренировку бессмысленно.
            ...
            if active:
                card = {
                    ...
                    "description": "Тренировка не была завершена. Продолжить или закрыть её?",
        ...
        if card is None:
            planned = db.execute(text("""
                SELECT id, title, exercises_ids FROM planned_workouts
                WHERE user_id = :uid AND DATE(planned_datetime) = :today AND status = 'pending'
                ORDER BY planned_datetime LIMIT 1
            """), {"uid": user.id, "today": today}).fetchone()
            if planned:
                ex_ids = planned.exercises_ids or []
                if isinstance(ex_ids, str):
                    import json as _j
                    try:
                        ex_ids = _j.loads(ex_ids)
                    except Exception:
                        ex_ids = []
                card = {
                    "state": "planned",
                    "title": planned.title or "Запланированная тренировка",
                    "description": "Ты запланировал эту тренировку на сегодня.",
                    "cta": "Начать тренировку",
                    "planned_workout_id": planned.id,
                    "exercises_count": len(ex_ids),
                }

        if card is None:
            options = _build_split_options(db, user.id, user.split_type)
            available = [o for o in options if not o["recently_trained"]] or options
            best = available[0] if available else None
            if best:
                # Текст на сегодня уже написан — берём его. Если нет, показываем
                # факты из правил сразу и ставим генерацию в фон: экран не ждёт AI.
                rec = db.execute(text("""
                    SELECT description, split_key FROM daily_recommendations
                    WHERE user_id = :uid AND rec_date = :d AND status = 'active'
                """), {"uid": user.id, "d": today}).fetchone()

                if rec and rec.description and rec.split_key == best["key"]:
                    descr = rec.description
                else:
                    descr = _rec_description(db, user.id, best, week)
                    if background is not None and not rec:
                        background.add_task(_generate_daily_recommendation,
                                            user.id, tg_id, best, week)
                ...
                card = {
                    ...
                    "cta": "Начать тренировку",
                    "split_key": best["key"],
                    "group_ids": best["group_ids"],
                    "est_minutes": 60 + 15 * (len(best["groups"]) - 1),
                }
            else:
                card = {
                    "state": "recommendation",
                    "title": "Свободная тренировка",
                    "description": "Выбери группы мышц сам — истории пока мало для рекомендации.",
                    "cta": "Начать тренировку",
                    "group_ids": [],
                }

        # ── Статистика дня ──────────────────────────────────────────────
        kcal_eaten = db.execute(text("""
            SELECT COALESCE(SUM(kcal), 0) FROM food_log
            WHERE user_id = :uid AND DATE(date) = :today
        """), {"uid": user.id, "today": today}).scalar() or 0
        # Расход складывается из двух источников: спорт и тренировки в зале.
        # Прежний главный экран суммировал оба, при переписывании зал выпал —
        # и после тренировки «Потрачено» оставалось нулём.
        kcal_sport = db.execute(text("""
            SELECT COALESCE(SUM(calories_burned), 0) FROM sport_sessions
            WHERE user_id = :uid AND session_date = :today
        """), {"uid": user.id, "today": today}).scalar() or 0
        kcal_gym = db.execute(text("""
            SELECT COALESCE(SUM(calories_burned), 0) FROM workouts
            WHERE user_id = :uid AND status = 'finished' AND DATE(date) = :today
        """), {"uid": user.id, "today": today}).scalar() or 0
        kcal_burned = int(kcal_sport) + int(kcal_gym)

        last_weight = db.execute(text("""
            SELECT weight FROM weight_log WHERE user_id = :uid
            ORDER BY logged_at DESC LIMIT 1
        """), {"uid": user.id}).scalar()

        # Дней с последнего чек-ина — главный экран подсказывает, когда пора
        last_checkin = db.execute(text("""
            SELECT MAX(DATE(created_at)) FROM checkins WHERE user_id = :uid
        """), {"uid": user.id}).scalar()
        days_since_checkin = (today - last_checkin).days if last_checkin else None

        goal = user.desired_result
        kcal_goal = 1600 if goal == "lose_weight" else 2800 if goal == "gain_muscle" else 2200
        ...
        return {
            ...
            "card": card,
            "stats": {
                "kcal_eaten": int(kcal_eaten),
                "kcal_goal": kcal_goal,
                "kcal_burned": int(kcal_burned),
                "burn_goal": 400,
                "weight": float(last_weight) if last_weight else (float(user.weight) if user.weight else None),
                "days_since_checkin": days_since_checkin,
            },
        }


@app.get("/api/home/{tg_id}/alternatives", dependencies=[Depends(verify_tg_user)])
def get_home_alternatives(tg_id: int):
    ...
    with SessionLocal() as db:
        ...
        for o in options:
            item = {
                ...
                "rest_days": None if o["min_rest_days"] >= 99 else o["min_rest_days"],
                "est_minutes": 60 + 15 * (len(o["groups"]) - 1),
            }
            item["done_date"] = o["done_date"]
            if o["recently_trained"]:
                done.append(item)
            else:
...
class TrainingPrefsRequest(BaseModel):
    target_workouts_per_week: Optional[int] = None
    split_type: Optional[str] = None


@app.put("/api/training-prefs/{tg_id}", dependencies=[Depends(verify_tg_user)])
def update_training_prefs(tg_id: int, req: TrainingPrefsRequest):
    """Частота тренировок и система сплита."""
    if req.split_type and req.split_type not in SPLIT_SYSTEMS:
        raise HTTPException(status_code=400, detail="Unknown split_type")
    if req.target_workouts_per_week is not None and not (1 <= req.target_workouts_per_week <= 7):
        raise HTTPException(status_code=400, detail="target_workouts_per_week must be 1..7")

    with SessionLocal() as db:
        user = db.execute(text("SELECT id FROM users WHERE telegram_id = :tg"), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        if req.target_workouts_per_week is not None:
            db.execute(text("UPDATE users SET target_workouts_per_week = :v WHERE id = :uid"),
                       {"v": req.target_workouts_per_week, "uid": user.id})
        if req.split_type:
            db.execute(text("UPDATE users SET split_type = :v WHERE id = :uid"),
                       {"v": req.split_type, "uid": user.id})
        db.commit()
    return {"ok": True}


@app.get("/api/workouts/{tg_id}/last-full", dependencies=[Depends(verify_tg_user)])
def get_last_full_workout(tg_id: int):
    """Последняя завершённая тренировка с весами — для кнопки «Повторить прошлую»."""
    with SessionLocal() as db:
        user = db.execute(text("SELECT id FROM users WHERE telegram_id = :tg"), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        ...
        rows = db.execute(text("""
            SELECT ws.exercise_name, ws.weight, ws.reps, ws.set_number,
                   e.id AS ex_id, mg.name AS group_name
            FROM workout_sets ws
            LEFT JOIN exercises e ON (e.id = ws.exercise_id OR (ws.exercise_id IS NULL AND LOWER(e.name) = LOWER(ws.exercise_name)))
            LEFT JOIN muscle_groups mg ON mg.id = e.muscle_group_id
            WHERE ws.workout_id = :wid
            ORDER BY ws.id
        """), {"wid": wk.id}).fetchall()
        ...
        for r in rows:
            key = r.exercise_name
            if key not in by_ex:
                by_ex[key] = {"exercise_id": r.ex_id, "name": key,
                              "group_name": r.group_name, "sets": []}
                order.append(key)
        ...
        return {
            ...
            "date": str(wk.date),
            "workout_id": wk.id,
            "exercises": [by_ex[k] for k in order],
        }


class LastSetRpeRequest(BaseModel):
    exercise_name: str
    rpe: int


@app.post("/api/workout/{tg_id}/last-set-rpe", dependencies=[Depends(verify_tg_user)])
def set_last_set_rpe(tg_id: int, req: LastSetRpeRequest):
    """Проставляет RPE последнему записанному подходу этого упражнения.
    Отдельным вызовом, потому что оценка усилия приходит уже после записи —
    во время отдыха, когда человек понял, тяжело ли дался подход."""
    if not (1 <= req.rpe <= 10):
        raise HTTPException(status_code=400, detail="rpe must be 1..10")
    with SessionLocal() as db:
        user = db.execute(text("SELECT id FROM users WHERE telegram_id = :tg"), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        row = db.execute(text("""
            ...
            JOIN workouts w ON w.id = ws.workout_id
            WHERE w.user_id = :uid AND LOWER(ws.exercise_name) = LOWER(:ex)
            ORDER BY ws.id DESC LIMIT 1
        """), {"uid": user.id, "ex": req.exercise_name}).fetchone()
        if not row:
            return {"ok": False}
        db.execute(text("UPDATE workout_sets SET rpe = :r WHERE id = :sid"),
                   {"r": req.rpe, "sid": row.id})
        db.commit()
    return {"ok": True}
...
def _check_age(age):
    """Возрастной порог. Приложение выдаёт тренировочные нагрузки и рекомендации
    по питанию, собирает сведения о здоровье и передаёт их внешнему AI-сервису —
    несовершеннолетним это не предназначено."""
    if age is None:
        return
    try:
        age = int(age)
    ...
    if age < MIN_AGE:
        raise HTTPException(status_code=403, detail=f"Minimum age is {MIN_AGE}")
    if age > 100:
        raise HTTPException(status_code=400, detail="Invalid age")


CONSENT_KINDS = ("terms", "basic", "health", "ai")


class ConsentRequest(BaseModel):
    # Необязательное: старый клиент шлёт только platform. Без значения по умолчанию
    # он получал бы 422 и человек застревал на экране согласия без выхода.
    kinds: Optional[List[str]] = None  # какие согласия подтверждает пользователь
    platform: Optional[str] = None   # ios / android / tdesktop / web — из Telegram WebApp
    version: Optional[str] = None    # редакция документа


class ConsentWithdrawRequest(BaseModel):
    kind: str                        # какое согласие отзывается


def _consent_state(row) -> dict:
    return {
        "terms": row.consent_terms_at.isoformat() if row.consent_terms_at else None,
        "basic": row.consent_basic_at.isoformat() if row.consent_basic_at else None,
        "health": row.consent_health_at.isoformat() if row.consent_health_at else None,
        "ai": row.consent_ai_at.isoformat() if row.consent_ai_at else None,
        "version": row.consent_version,
        "platform": row.consent_platform,
        "username": row.consent_username,
    }


def _read_consents(db, user_id: int):
    return db.execute(text("""
        SELECT consent_terms_at, consent_basic_at, consent_health_at, consent_ai_at,
               consent_version, consent_platform, consent_username
        FROM users WHERE id = :uid
    """), {"uid": user_id}).fetchone()


def _require_consent(db, user_id: int, kind: str):
    """Отказывает в операции, если соответствующее согласие не дано или отозвано.

    Документ обещает, что после отказа от передачи в Anthropic AI-функции
    недоступны, а сведения о здоровье не обрабатываются. Без этой проверки
    обещание расходилось бы с поведением приложения.
    """
    row = _read_consents(db, user_id)
    col = {"terms": row.consent_terms_at, "basic": row.consent_basic_at,
           "health": row.consent_health_at, "ai": row.consent_ai_at}.get(kind)
    if not col:
        raise HTTPException(status_code=451, detail=f"Consent required: {kind}")


@app.get("/api/consent/{tg_id}", dependencies=[Depends(verify_tg_user)])
def get_consent(tg_id: int):
    with SessionLocal() as db:
        user = db.execute(text("SELECT id FROM users WHERE telegram_id = :tg"), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        return {"consents": _consent_state(_read_consents(db, user.id)),
                "current_version": CONSENT_VERSION, "min_age": MIN_AGE}


@app.post("/api/consent/{tg_id}", dependencies=[Depends(verify_tg_user)])
def give_consent(tg_id: int, req: ConsentRequest):
    """Фиксирует выбранные согласия. Каждое — отдельной отметкой и записью в журнал."""
    try:
        return _give_consent_impl(tg_id, req)
    except HTTPException:
        raise
    except Exception as e:
        # Без этого браузер видит только 500 и жалуется на CORS, а настоящая
        # причина не попадает никуда: у 500 нет заголовков CORS, и текст теряется.
        logger.exception(f"[consent] ошибка при записи согласия tg_id={tg_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Consent save failed: {e}")


def _give_consent_impl(tg_id: int, req: ConsentRequest):
    # Старый клиент не передаёт состав — засчитываем базовый набор, как было раньше
    requested = req.kinds if req.kinds is not None else ["terms", "basic"]
    kinds = [k for k in requested if k in CONSENT_KINDS]
    if not kinds:
        raise HTTPException(status_code=400, detail="No valid consent kinds")

    ver = (req.version or CONSENT_VERSION)[:20]
    plat = (req.platform or "unknown")[:60]

    with SessionLocal() as db:
        user = db.execute(text("""
            SELECT id, username FROM users WHERE telegram_id = :tg
        """), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        uname = (user.username or "")[:100] or None

        for k in kinds:
            db.execute(text(f"""
                UPDATE users SET consent_{k}_at = NOW(), consent_version = :v,
                                 consent_platform = :p, consent_username = :u
                WHERE id = :uid
            """), {"v": ver, "p": plat, "u": uname, "uid": user.id})
            db.execute(text("""
                INSERT INTO consent_log (user_id, kind, action, version, platform, username)
                VALUES (:uid, :k, 'given', :v, :p, :u)
            """), {"uid": user.id, "k": k, "v": ver, "p": plat, "u": uname})

        # Совместимость с прежней схемой: общая отметка = базовый набор дан
        if "terms" in kinds or "basic" in kinds:
            db.execute(text("""
                UPDATE users SET consent_given_at = COALESCE(consent_given_at, NOW()) WHERE id = :uid
            """), {"uid": user.id})
        db.commit()
        logger.info(f"[consent] tg_id={tg_id} подтвердил: {kinds}")
        return {"ok": True, "consents": _consent_state(_read_consents(db, user.id))}


@app.post("/api/consent/{tg_id}/withdraw", dependencies=[Depends(verify_tg_user)])
def withdraw_consent(tg_id: int, req: ConsentWithdrawRequest):
    """Частичный отзыв одного согласия."""
    ...
    with SessionLocal() as db:
        ...
        db.execute(text(f"UPDATE users SET consent_{req.kind}_at = NULL WHERE id = :uid"),
                   {"uid": user.id})
        db.execute(text("""
            INSERT INTO consent_log (user_id, kind, action, version, username)
            VALUES (:uid, :k, 'withdrawn', :v, :u)
        """), {"uid": user.id, "k": req.kind, "v": CONSENT_VERSION,
               "u": (user.username or "")[:100] or None})

        # Отзыв согласия на здоровье стирает сами сведения: хранить их больше не на чем
        if req.kind == "health":
            db.execute(text("""
                UPDATE users SET medical_conditions = NULL, allergies = NULL, medications = NULL,
                                 has_heart_condition = FALSE, has_joint_issues = FALSE,
...
# Таблицы с данными пользователя. Список формируется не вручную, а сверяется
# с реальной схемой при вызове: если появится новая таблица с user_id, она
# попадёт в удаление автоматически и данные не останутся висеть после ухода.
_USER_TABLES_ORDER = [
    "workout_sets",          # сначала дети, потом родители
    "workouts",
    "planned_workouts",
...
def _user_linked_tables(db) -> list:
    """Все таблицы с колонкой user_id — из схемы, а не из захардкоженного списка."""
    rows = db.execute(text("""
        SELECT table_name FROM information_schema.columns
        WHERE table_schema = 'public' AND column_name = 'user_id'
    """)).fetchall()
    found = {r[0] for r in rows}
    ordered = [t for t in _USER_TABLES_ORDER if t in found]
    # Таблицы, которых нет в известном порядке, удаляем после него
    return ordered + sorted(found - set(ordered))


@app.get("/api/account/{tg_id}/export", dependencies=[Depends(verify_tg_user)])
def export_account(tg_id: int):
    """Выгрузка всех данных пользователя — право на получение своих данных."""
    with SessionLocal() as db:
        user = db.execute(text("SELECT * FROM users WHERE telegram_id = :tg"), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        ...
        data = {"profile": {k: _ser(v) for k, v in dict(user._mapping).items()}}

        for tbl in _user_linked_tables(db):
            if tbl == "workout_sets":
                continue  # выгружаем вместе с тренировками ниже
            try:
                rows = db.execute(text(f"SELECT * FROM {tbl} WHERE user_id = :uid"),
                                  {"uid": user.id}).fetchall()
                data[tbl] = [{k: _ser(v) for k, v in dict(r._mapping).items()} for r in rows]
            except Exception as e:
                logger.warning(f"[export] пропущена таблица {tbl}: {e}")

        try:
            sets = db.execute(text("""
                SELECT ws.* FROM workout_sets ws
                JOIN workouts w ON w.id = ws.workout_id
                WHERE w.user_id = :uid ORDER BY ws.id
            """), {"uid": user.id}).fetchall()
            data["workout_sets"] = [{k: _ser(v) for k, v in dict(r._mapping).items()} for r in sets]
        except Exception as e:
            logger.warning(f"[export] подходы не выгружены: {e}")

    return {"exported_at": datetime.utcnow().isoformat(), "data": data}


@app.delete("/api/account/{tg_id}", dependencies=[Depends(verify_tg_user)])
def delete_account(tg_id: int, confirm: str = ""):
    """Полное удаление аккаунта и всех связанных данных.

    Требует confirm=DELETE в запросе: удаление необратимо, и случайный вызов
    не должен стирать историю за месяцы.
    """
    if confirm != "DELETE":
    ...
    with SessionLocal() as db:
        ...
        deleted = {}
        # Подходы висят на тренировках, а не на пользователе — удаляем первыми
        try:
            r = db.execute(text("""
            ...
            """), {"uid": user.id})
            deleted["workout_sets"] = r.rowcount
        except Exception as e:
            logger.warning(f"[delete] workout_sets: {e}")

        for tbl in _user_linked_tables(db):
            if tbl == "workout_sets":
                continue
            try:
                r = db.execute(text(f"DELETE FROM {tbl} WHERE user_id = :uid"), {"uid": user.id})
                deleted[tbl] = r.rowcount
            except Exception as e:
                logger.warning(f"[delete] {tbl}: {e}")

        db.execute(text("DELETE FROM users WHERE id = :uid"), {"uid": user.id})
        deleted["users"] = 1
        db.commit()

    logger.info(f"[delete] Аккаунт tg_id={tg_id} удалён: {deleted}")
    return {"ok": True, "deleted": {k: v for k, v in deleted.items() if v}}



@app.post("/api/workout/{tg_id}/close-active", dependencies=[Depends(verify_tg_user)])
def close_active_workout(tg_id: int):
    """Закрывает незавершённую тренировку по кнопке на главном экране.
    Пустую удаляем, с подходами помечаем завершённой — подходы были сделаны реально."""
    with SessionLocal() as db:
        user = db.execute(text("SELECT id FROM users WHERE telegram_id = :tg"), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        ...
        if wk.sets == 0:
            ...
            db.execute(text("UPDATE workouts SET status='finished' WHERE id = :wid"), {"wid": wk.id})
            action = "finished"
        db.commit()
    return {"ok": True, "action": action, "sets": wk.sets}


def _personal_rest_sec(db, user_id: int, exercise_name: str):
    """Медианная пауза между подходами этого упражнения у этого человека.
    Считается по времени записи подходов; пока истории мало — возвращает None,
    и фронтенд берёт значение по группе мышц."""
    try:
        rows = db.execute(text("""
            SELECT ws.workout_id, ws.created_at
            FROM workout_sets ws
            JOIN workouts w ON w.id = ws.workout_id
            WHERE w.user_id = :uid AND LOWER(ws.exercise_name) = LOWER(:ex)
              AND ws.created_at IS NOT NULL
            ORDER BY ws.workout_id, ws.created_at
        """), {"uid": user_id, "ex": exercise_name}).fetchall()

        gaps = []
        prev_wid, prev_ts = None, None
        for r in rows:
            if prev_wid == r.workout_id and prev_ts:
                g = (r.created_at - prev_ts).total_seconds()
                # Отсекаем мусор: меньше 20 секунд — двойное нажатие,
                # больше 10 минут — человек отвлёкся, это не пауза между подходами
                if 20 <= g <= 600:
                    gaps.append(g)
            prev_wid, prev_ts = r.workout_id, r.created_at

        if len(gaps) < 3:
            return None
        gaps.sort()
        med = gaps[len(gaps) // 2]
        return int(round(med / 15) * 15)  # округляем до 15 секунд
    except Exception as e:
        logger.warning(f"_personal_rest_sec error: {e}")
        return None


@app.get("/api/exercises/{tg_id}/last-sets", dependencies=[Depends(verify_tg_user)])
def get_last_exercise_sets(tg_id: int, exercise_name: str):
    """Последний результат по конкретному упражнению (вес × повторы) — используется на карточке
    упражнения как fallback, если совета AI-тренера по этому упражнению нет."""
    with SessionLocal() as db:
        user = db.execute(text("SELECT id FROM users WHERE telegram_id=:tg"), {"tg": tg_id}).fetchone()
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        last_wid = db.execute(text("""
            SELECT ws.workout_id, w.date
            FROM workout_sets ws JOIN workouts w ON w.id = ws.workout_id
            WHERE w.user_id=:uid AND ws.exercise_name=:ex AND w.status='finished'
            ORDER BY w.date DESC LIMIT 1
        """), {"uid": user[0], "ex": exercise_name}).fetchone()
        if not last_wid:
            return {"found": False}
        sets = db.execute(text("""
            SELECT weight, reps FROM workout_sets
            WHERE workout_id=:wid AND exercise_name=:ex ORDER BY id
        """), {"wid": last_wid[0], "ex": exercise_name}).fetchall()
        return {
            "found": True,
            "date": str(last_wid[1]),
            "rest_sec": _personal_rest_sec(db, user[0], exercise_name),
            "sets": [{"weight": float(s[0]) if s[0] else 0, "reps": s[1] or 0} for s in sets if (s[1] or 0) > 0],
        }
...
@app.get("/api/user/{tg_id}/supplements", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/user/{tg_id}/supplements", dependencies=[Depends(verify_tg_user)])
...
@app.delete("/api/user/{tg_id}/supplements/{supp_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/progress/muscle-groups/{tg_id}", dependencies=[Depends(verify_tg_user)])
def get_muscle_group_progress(tg_id: int, days: int = 90):
    ...
    with SessionLocal() as db:
        ...
        rows = db.execute(text(f"""
            ...
            GROUP BY w.id, DATE(w.date), mg.id, mg.name, mg.emoji, mg.icon_ext
        ...
        for r in rows:
            ...
            if g not in by_group:
                by_group[g] = {"emoji": r.group_emoji or "", "icon_url": r2_group_icon_url(r.mg_id, r.group_icon_ext) if r.mg_id else None, "workouts": []}
...
@app.get("/api/plateau/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/referral/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/referral/apply", dependencies=[Depends(verify_tg_user)])
...
def check_and_grant_achievements(db, user_id: int, workout_id: int = None) -> list:
    """Проверяет и выдаёт новые достижения. Возвращает список новых.
    workout_id (опционально) — если передан ID только что завершённой тренировки,
    дополнительно проверяются личный рекорд и прирост рабочего веса на 5кг+."""
    # Уже выданные
    ...
    stats = db.execute(text("""
        SELECT
            ...
            (SELECT COUNT(*) FROM sport_sessions WHERE user_id=:uid) as sport_count,
    ...
    checks = [
        ...
        ("sport_first",    stats.sport_count >= 1),
        ("sport_10",       stats.sport_count >= 10),
        ("checkin_10",     stats.checkin_count >= 10),
    ]

    # Личный рекорд и прирост рабочего веса — только если проверяем конкретную завершённую тренировку.
    # Раньше рекорд детектился и слался уведомлением в Telegram (check_records_after_workout в gymbot.py),
    # но само достижение никогда не выдавалось — эта проверка была полностью независимой от этой функции.
    if workout_id and ("personal_record" not in granted_ids or "weight_5kg" not in granted_ids):
        try:
            current_maxes = db.execute(text("""
                SELECT exercise_name, MAX(weight) as max_weight
                FROM workout_sets
                WHERE workout_id=:wid AND weight IS NOT NULL AND weight > 0
                GROUP BY exercise_name
            """), {"wid": workout_id}).fetchall()

            has_new_record = False
            has_5kg_jump = False
            for row in current_maxes:
                ex_name, cur_max = row[0], float(row[1] or 0)
                if cur_max <= 0:
                    continue
                prev_max = db.execute(text("""
                    SELECT MAX(ws.weight) FROM workout_sets ws
                    JOIN workouts w ON w.id = ws.workout_id
                    WHERE w.user_id=:uid AND ws.exercise_name=:ex
                      AND ws.workout_id != :wid AND ws.weight IS NOT NULL
                """), {"uid": user_id, "ex": ex_name, "wid": workout_id}).scalar()
                prev_max = float(prev_max or 0)
                if prev_max > 0 and cur_max > prev_max:
                    has_new_record = True
                    if cur_max - prev_max >= 5:
                        has_5kg_jump = True

            if has_new_record:
                checks.append(("personal_record", True))
            if has_5kg_jump:
                checks.append(("weight_5kg", True))
        except Exception as e:
            logger.warning(f"Ошибка проверки личного рекорда: {e}")
...
@app.get("/api/achievements/{tg_id}", dependencies=[Depends(verify_tg_user)])
    ...
    with SessionLocal() as db:
        ...
        if tg_id:
            ...
            if me:
                ...
                """), {"pts": int(me[2])}).scalar()
                my_position = int(pos)
                my_data = {"name": me[1], "pts": int(me[2]),
        ...
        for i, r in enumerate(rows):
            leaders.append({
                "position": i + 1,
                # telegram_id и username наружу не отдаём: по ним можно найти
                # и написать человеку. Для подсветки своей строки хватает is_me.
                "name": r[1] or "Атлет",
                "pts": int(r[3]),
...
@app.get("/api/strength-norms/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/sport/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/sport/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.delete("/api/sport/{tg_id}/{session_id}", dependencies=[Depends(verify_tg_user)])
...
@app.get("/api/custom-exercises/{tg_id}", dependencies=[Depends(verify_tg_user)])
...
@app.post("/api/custom-exercises/{tg_id}", status_code=201, dependencies=[Depends(verify_tg_user)])
...
@app.delete("/api/custom-exercises/{tg_id}/{ex_id}", dependencies=[Depends(verify_tg_user)])
...
STREAK_REWARDS = [
    ...
    {"days": 90, "name": "Квартал",         "emoji": "👑",  "pts": 300, "desc": "90 дней — ты абсолютная легенда"},
]

def calculate_streak(db, user_id: int) -> int:
    """Считает текущую серию активных дней с допуском пропуска до 2 дней подряд
    (например выходные) — серия не сгорает от одного-двух дней без активности."""
    rows = db.execute(text("""
        SELECT DISTINCT DATE(date) as d FROM workouts
        WHERE user_id=:uid AND status='finished'
        UNION
    ...
    GRACE_DAYS = 2  # допускаем пропуск до 2 дней подряд, серия не сгорает
    ...
    today = date.today()
    if (today - dates[0]).days > GRACE_DAYS + 1:
        return 0  # последняя активность была слишком давно — серия уже прервана
...
@app.get("/api/gamification/{tg_id}", dependencies=[Depends(verify_tg_user)])
