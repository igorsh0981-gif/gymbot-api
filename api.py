...
def verify_tg_user(tg_id: int, x_telegram_init_data: Optional[str] = Header(None)) -> int:
    ...
    Без валидного initData или при несовпадении id — 403."""
    if not BOT_TOKEN_FOR_AUTH:
        # Токен не настроен на этом окружении — не блокируем (например, локальная разработка),
        # но пишем предупреждение, чтобы это не осталось незамеченным в проде.
        logger.warning("[auth] BOT_TOKEN not set — initData verification disabled!")
        return tg_id
