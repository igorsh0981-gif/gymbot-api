...
def update_streak(db, uid: int):
    ...
    GRACE_DAYS = 2  # допускаем пропуск до 2 дней подряд (выходные/отдых), серия не сгорает

    if last_date == today:
        new_streak = cur_streak
    elif last_date and (today - last_date).days <= GRACE_DAYS + 1:
        # Тренировка была не позже чем GRACE_DAYS+1 дней назад -> серия продолжается,
        # даже если 1-2 дня подряд пропущены (выходные/отдых)
...
async def ai_ask(req: AIRequest):
    ...
    if user:
        context = (
            ...
            f"streak={getattr(user, 'current_streak', 0)} days in a row. "
            f"{lang_hint}"
            f"{tone_prompt}"
            f"{checkin_context}"
            f"{workouts_context}"
...
def update_food_log(tg_id: int, log_id: int, req: FoodLogRequest):
    ...
    with SessionLocal() as db:
        ...
        """), {"name": req.meal_name[:200], "kcal": req.kcal,
               "protein": req.protein or 0, "fat": req.fat or 0, "carb": req.carb or 0,
               "log_id": log_id, "uid": user.id})
        db.commit()
