...
async def voice_transcribe(
    ...
    try:
        ...
        if len(audio_bytes) > 25 * 1024 * 1024:  # 25MB лимит
        ...
        logger.info(f"[Voice] User {tg_id}: transcribed {len(audio_bytes)//1024}kb → {len(text)} chars")
        return {"text": text, "duration_approx": len(audio_bytes) / 16000}
...
def get_plateau_status(tg_id: int):
    ...
    with SessionLocal() as db:
        ...
        # Прогресс по упражнениям за 3 недели
        stagnant_exercises = db.execute(text("""
            ...
            WHERE w.user_id=:uid
              ...
              AND w.date >= NOW() - INTERVAL '21 days'
            GROUP BY ws.exercise_name
            HAVING COUNT(DISTINCT DATE(w.date)) >= 3
               AND MAX(ws.weight) - MIN(ws.weight) < 2.5
