...
# Мониторинг ошибок. Включается только если задан SENTRY_DSN и установлен пакет —
# без них приложение работает как раньше, без падений на импорте.
try:
    import sentry_sdk as _sentry
    _SENTRY_DSN = os.getenv("SENTRY_DSN", "")
    if _SENTRY_DSN:
        _sentry.init(dsn=_SENTRY_DSN, traces_sample_rate=0.0, send_default_pii=False,
                     environment=os.getenv("RAILWAY_ENVIRONMENT_NAME", "production"))
        logger.info("[sentry] мониторинг ошибок включён")
except ImportError:
    pass
...
def give_consent(tg_id: int, req: ConsentRequest):
    ...
    Пишем отметку в профиль: когда дано, какая редакция принята, с какого
    устройства и под каким логином — чтобы факт согласия можно было показать
    самому пользователю и подтвердить при разбирательстве.
    """
    with SessionLocal() as db:
        user = db.execute(text("""
            SELECT id, username FROM users WHERE telegram_id = :tg
        ...
        db.execute(text("""
            UPDATE users SET consent_given_at = NOW(), consent_version = :v,
                             consent_platform = :p, consent_username = :u
            WHERE id = :uid
        """), {"v": (req.version or CONSENT_VERSION)[:20],
               "p": (req.platform or "unknown")[:60],
               "u": (user.username or "")[:100] or None,
               "uid": user.id})
        db.commit()

        row = db.execute(text("""
            SELECT consent_given_at, consent_version, consent_platform, consent_username
            FROM users WHERE id = :uid
        """), {"uid": user.id}).fetchone()

    return {"ok": True,
            "consent_given_at": row.consent_given_at.isoformat() if row.consent_given_at else None,
            "consent_version": row.consent_version,
            "consent_platform": row.consent_platform,
            "consent_username": row.consent_username}
