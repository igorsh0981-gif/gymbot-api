...
def get_nutrition(tg_id: int):
    with SessionLocal() as db:
        ...
        logs = db.execute(text("""
            SELECT id, meal_name, kcal, protein, fat, carb, date FROM food_log
...
class FoodPhotoRequest(BaseModel):
    image_base64: str      # base64 фото
    tg_id: Optional[int] = None
    save: Optional[bool] = False  # сразу сохранить в food_log
    ...
    if req.save and req.tg_id:
        try:
            with SessionLocal() as db:
                ...
                if user:
                    db.execute(text("""
                        INSERT INTO food_log (user_id, date, meal_name, kcal, protein, fat, carb)
                        VALUES (:uid, :now, :name, :kcal, :protein, :fat, :carb)
                    """), {
                        "uid": user.id,
                        "now": datetime.utcnow().replace(tzinfo=None),
