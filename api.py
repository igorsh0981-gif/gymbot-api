...
# Streak rewards: растущие бонусы за серию дней подряд с тренировкой
STREAK_MILESTONES = {3: 20, 7: 50, 14: 120, 30: 300}

def update_streak(db, uid: int):
    """
    Обновляет серию дней подряд при завершении тренировки.
    Вызывать внутри открытой транзакции, ДО db.commit().
    Возвращает dict: {current_streak, longest_streak, milestone_hit, milestone_points}
    """
    row = db.execute(text("""
        SELECT current_streak, longest_streak, last_workout_date, streak_milestones_claimed
        FROM users WHERE id=:uid
    """), {"uid": uid}).fetchone()
    if not row:
        return {"current_streak": 0, "longest_streak": 0, "milestone_hit": None, "milestone_points": 0}

    today = datetime.utcnow().replace(tzinfo=None).date()
    cur_streak = row[0] or 0
    longest = row[1] or 0
    last_date = row[2]
    claimed = (row[3] or "").strip()
    claimed_set = set(int(x) for x in claimed.split(",") if x.strip().isdigit())
    ...
    milestone_hit = None
    milestone_points = 0
    if new_streak in STREAK_MILESTONES and new_streak not in claimed_set:
        milestone_hit = new_streak
        milestone_points = STREAK_MILESTONES[new_streak]
        claimed_set.add(new_streak)

    db.execute(text("""
        UPDATE users SET current_streak=:cs, longest_streak=:ls,
               last_workout_date=:ld, streak_milestones_claimed=:claimed
        WHERE id=:uid
    """), {
        "cs": new_streak, "ls": new_longest, "ld": today,
        "claimed": ",".join(str(x) for x in sorted(claimed_set)), "uid": uid
    })

    if milestone_points > 0:
        add_points(db, uid, milestone_points, f"streak_{milestone_hit}_days")

    return {
        "current_streak": new_streak, "longest_streak": new_longest,
        "milestone_hit": milestone_hit, "milestone_points": milestone_points
    }
...
async def ai_ask(req: AIRequest):
            ...
            try:
                ...
                if recent_wk:
                    ...
                    for wk in recent_wk:
                        ...
                        sets_str = "; ".join(n + ": " + ', '.join(v) for n,v in ex_grouped.items())
    ...
    if user:
        context = (
            f"You are GymBot AI Coach — a personal trainer who knows this client deeply. "
            f"Client profile: {user.age}y, {user.weight}kg, {user.height}cm, "
            f"level={user.fitness_level}, goal={user.desired_result}.{medical_str}{allergies_str} {lang_hint} "
            f"Be practical, max 400 words. "
            f"Use RPE (Rate of Perceived Exertion 1-10) and RIR (Reps in Reserve) when relevant: "
            f"RPE 7=3 RIR (easy), RPE 8=2 RIR (moderate), RPE 9=1 RIR (hard), RPE 10=0 RIR (max effort). "
            f"Mention progressive overload, deload weeks, and periodization when giving workout advice. "
            f"STRICT WORKOUT RULES: "
            f"1) Single muscle group workout (chest/legs/shoulders/back/arms): MAX 5 exercises total. "
            f"2) Combined workout (e.g. biceps+triceps, legs+glutes, chest+shoulders): MAX 6 exercises total across ALL groups. "
            f"Never exceed these limits regardless of user request. Quality over quantity. "
            f"REALISTIC PROGRESSION RULES (CRITICAL): "
            f"Never suggest +2.5kg per week for upper body isolation exercises (flyes, laterals, pullovers, curls) — realistic is +1-2kg per 2-4 WEEKS. "
            f"For compound lifts (bench, squat, deadlift, row): +2.5kg per 1-2 weeks is realistic for intermediate. "
            f"For advanced athletes (like this client): progression is +1.25-2.5kg per MONTH on most lifts. "
            f"Suggest deload when client has been training hard for 3+ weeks. "
            f"Focus on TECHNIQUE and FEEL (RPE) before adding weight. "
            f"If client hit all reps with good form at RPE 8 — THEN suggest weight increase. "
            f"Never suggest weight increases that would demotivate — be conservative and realistic."
            f"{tone_prompt}"
            f"{checkin_context}"
            f"{workouts_context}"
            f"{nutrition_context}"
...
def finish_workout(workout_id: int, req: FinishWorkoutRequest, tg_id: int):
    with SessionLocal() as db:
        ...
        streak_info = update_streak(db, user.id)
        ...
        return {
            ...
            "streak_milestone_hit": streak_info["milestone_hit"],
            "streak_milestone_points": streak_info["milestone_points"],
        }
...
def check_streak_rewards(db, user_id: int, streak: int):
    """Выдаёт награды за новые milestone стриков."""
