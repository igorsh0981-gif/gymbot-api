    ...
    with SessionLocal() as db:
        rows = db.execute(text("""
            SELECT telegram_id, first_name, username,
                   ...
                   COALESCE(streak_days,0) as streak
            FROM users
...
STRENGTH_NORMS_MALE = {
    "Жим штанги лёжа":       [0.5, 1.0, 1.25, 1.5],
    "Приседания со штангой":  [0.75, 1.25, 1.5, 1.75],
    "Становая тяга":          [1.0, 1.5, 2.0, 2.5],
    "Жим штанги стоя":        [0.35, 0.65, 0.85, 1.0],
    "Подтягивания":           [1, 5, 10, 15],  # кол-во повторений
}
...
def get_strength_norms(tg_id: int):
    ...
    with SessionLocal() as db:
        ...
        for exercise, levels in STRENGTH_NORMS_MALE.items():
            adj = [round(l * bw * age_coef, 1) if i < 3 else l for i, l in enumerate(levels)]
