...
async def ai_ask(req: AIRequest):
    ...
    with SessionLocal() as db:
        ...
        if user:
            ...
            if requests_today >= AI_DAILY_LIMIT and not req.skip_limit and not is_admin:
        ...
        # Загружаем последний чек-ин пользователя (вес + сон) для AI контекста
        checkin_context = ""
        if user:
            try:
                ...
                if last_ci:
                    ...
                    if ci_parts:
                        ...
                        checkin_context = f"\n\nLATEST CHECK-IN ({days_ago}d ago): {', '.join(ci_parts)}. Use this to personalize advice — consider recovery state, sleep and energy when recommending intensity."
            except Exception:
                pass

        # Загружаем список упражнений из каталога для контекста AI
        exercises_context = ""
        try:
            ...
            if ex_rows:
                ...
                exercises_context += "\n".join(
                    f"{g}: {', '.join(exs)}" for g, exs in by_group.items()
                )
                exercises_context += "\n\nIMPORTANT: When recommending a workout plan, ONLY use exercises from the catalog above. List exercise names exactly as shown."
        except Exception:
            pass

        # Определяем тип вопроса — тренировка или общий
        workout_keywords = ["тренировк", "программ", "сплит", "план", "упражнен", "workout", "program", "split", "план"]
        ...
        if user:
            ...
            if is_workout_question:
                context += (
                    "\n\nIf you provide a workout plan, end your response with a special JSON block:\n"
                    "WORKOUT_PLAN_JSON: {\"title\": \"название тренировки\", \"exercises\": [\"Упражнение 1\", \"Упражнение 2\"]}\n"
        ...
        # Извлекаем workout_plan если есть
        workout_plan = None
        ...
        if user:
            ...
            db.commit()
        result = {"answer": answer, "requests_used": requests_today + 1, "workout_plan": workout_plan}
...
def create_planned(tg_id: int, req: PlannedWorkoutRequest):
    with SessionLocal() as db:
        ...
        """), {"uid": user.id, "dt": dt, "title": req.title[:200],
               "exids": _json_pw.dumps(req.exercise_ids or [])})
        db.commit()
        return {"ok": True}
...
def update_planned(tg_id: int, workout_id: int, req: UpdatePlannedRequest):
    with SessionLocal() as db:
        ...
        if req.planned_datetime:
            ...
            params["dt"] = datetime.fromisoformat(req.planned_datetime)
...
def add_food_log(tg_id: int, req: FoodLogRequest):
    with SessionLocal() as db:
        ...
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
        db.execute(text("""
            INSERT INTO food_log (user_id, meal_name, kcal, protein, fat, carb, logged_at)
            VALUES (:uid, :name, :kcal, :protein, :fat, :carb, :now)
